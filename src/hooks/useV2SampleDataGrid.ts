...
export const useV2SampleDataGrid = ({
    ...
    // Load data when sampleDataArray changes
    useEffect(() => {
        if (sampleDataArray && sampleDataArray.length > 0) {
            const transformedData = transformV2SampleData(sampleDataArray);
    ...
    const handleAddRow = useCallback(() => {
        // Create a new row with dynamic fields based on the original data structure
        const newRow = createEmptyRow(sampleDataArray || []);
            ...
            setTimeout(() => {
                if (gridRef.current?.api) {
                    ...
                    const firstEditableColumn = columnDefs?.find(col => {
                        ...
                        return 'field' in col && 'editable' in col && col.editable !== false && col.field;
                    }) as ColDef | undefined;
